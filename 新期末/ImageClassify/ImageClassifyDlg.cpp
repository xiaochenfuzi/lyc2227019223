...
BEGIN_MESSAGE_MAP(CImageClassifyDlg, CDialogEx)
	...
	ON_EN_CHANGE(IDC_EDIT_BACKLINGHTING_DIR, &CImageClassifyDlg::OnEnChangeEditBacklinghtingDir)
END_MESSAGE_MAP()
...
void CImageClassifyDlg::OnBnClickedButtonStartSingle()
{
	...
	{
		pEditResult->SetWindowText(L"正常图像");
	}else if (type == AG_IMAGE)
	{
		pEditResult->SetWindowText(L"过暗图像");
	}
	else if (type == NG_IMAGE)
	{
		pEditResult->SetWindowText(L"逆光图像");
	}
	else if (type == FG_IMAGE)
	{
		pEditResult->SetWindowText(L"反光图像");
	}
}
...
void CImageClassifyDlg::OnEnChangeEditBacklinghtingDir()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 CDialogEx::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
